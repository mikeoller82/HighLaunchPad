...
export default function NewsletterEditorPage() {
  ...
  const template = getNewsletterTemplateById(params?.newsletterId || 'default');
  ...
  const [newsletterInfo, setNewsletterInfo] = useState({
    ...
    slug: params?.newsletterId || 'default',
    subject: "This week's news!",
  });
  const [newsletterContext, setNewsletterContext] = useState('');
  const [showPreview, setShowPreview] = useState(false);

  const contentManager = useContentManager('newsletter', params?.newsletterId || 'default');
