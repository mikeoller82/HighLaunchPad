...
const HeroPreview = ({ content, styles, buttonStyles }: { content: any, styles: any, buttonStyles: any }) => (
  <div className="p-8 text-center rounded-lg" style={{ color: styles.textColor }}>
    ...
    >{typeof content.cta === 'string' ? content.cta : content.cta?.primary || 'Get Started'}</Button>
  </div>
);

const ImagePreview = ({ content }: { content: any }) => {
    const [imgSrc, setImgSrc] = useState(content.src || '/images/placeholder.jpg');
    return (
        <div className="py-8">
...
const WebsitePreviewPage = () => {
  ...
  // Always use the production renderer for preview/live view
  if (!params || !params?.websiteId) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Invalid Website ID</h1>
          <p className="text-gray-600">The website you&apos;re looking for could not be found.</p>
        </div>
      </div>
    );
  }
  
  return <WebsiteBuilder templateId={params?.websiteId || 'default'} renderComponent={renderWebsiteComponent} />;
};
